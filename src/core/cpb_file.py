...
# ref: https://blog.csdn.net/weixin_34351321/article/details/91696169
# ref: https://github.com/ColdWindScholar/cpbtool
# warning: only v1.7 supported.
import os
from ctypes import LittleEndianStructure, sizeof, memmove, string_at, addressof, byref, c_uint, c_ulonglong, c_char, \
    c_ushort, c_uint64, c_uint8, c_uint16, c_uint32
...
# 1.7 header
class CpbHeader(BasicStruct):
    _fields_ = [
        ("cp_magic", c_char * 2),
        ("cp_version", c_uint8 * 2),
        ("correct", c_uint16 * 6),
        ("unknown", c_uint8 * 32),
        ("model", c_char * 32),
        ("hardware_version", c_char * 16),
        ("version", c_char * 64),
        ("rom_from", c_char * 256),
        ("img_header_end_pos", c_uint32),
        ("reverse", c_uint8 * 128),
        ("checksum", c_uint32)
    ]


class ImageHeader(BasicStruct):
    _fields_ = [
        ("filename", c_char * 64),
        ("offset", c_uint32),
        ("size", c_uint32),
...
def crc16(pdata: bytes, nsize: int, uicrc: int) -> int:
    i: int = 0
    while i < nsize:
        uicrc = (uicrc << 8) ^ CRC16Tab[(uicrc >> 8) ^ pdata[i]]
        i += 1
    return uicrc


def check_header(header: CpbHeader) -> bool:
    if header.cp_magic != CP_MAGIC:
        print(f"[{header.cp_magic}] Not a cpb file.")
        return False
    if header.cp_version[:] != [1, 7]:
        print(f"[{header.cp_version[:]}] only 1.7 supported.")
        return False
    return True


def extract_cpb(filepath: str, output_folder: str):
    ...
    with open(filepath, "rb") as f:
        ...
        for i in range(nImgCount):
            ...
            img_header.unpack(img_d)
            file_name = img_header.filename.decode()
            print(f"[{i + 1}/{nImgCount}] Extracting {file_name} ...")
            if img_header.size == 0:
                continue
            origin_seek = f.tell()
            f.seek(img_header.offset + header_offset)
