...
int e2fsdroid(e2fsdroid_args_struct arguments)
{
	...
	if (arguments.timestamp) {
		fixed_time = arguments.timestamp;
		android_configure = 1;
	}
	if (strcmp(arguments.fs_config, "") != 0) {
		fs_config_file = absolute_path(arguments.fs_config);
		android_configure = 1;
	}
	...
	if (strcmp(arguments.product_out, "") != 0) {
		product_out = absolute_path(arguments.product_out);
		android_configure = 1;
	}
