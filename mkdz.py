...
class MKDZFile(dz.DZFile):
    ...
    def loadParams(self):
        ...
        if params['formatMinor'] != 1:
            print("[!] Warning: File appears to be a v{:d}.{:d} format file, compatibility uncertain!".format(
                params['format_major'], params['format_minor']), file=sys.stderr)
    ...
    def loadChunks(self):
        ...
        self.chunks.sort(
            key=lambda c: (c.getStart() + (c.getDev() << 48) + (1 << 56 if c.chunkName[-4:] == ".img" else 0)))
...
class MKDZFileTools:
    ...
    def parseArgs(self):
        ...
        group.add_argument('-m', '--make', help='make DZ file from chunks in directory', action='store_true',
                           dest='createFile')
